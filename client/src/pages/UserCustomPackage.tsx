...
const UserCustomNavbar = () => {
  ...
  const fetchCitiesInIndia = async () => {
    try {
      ...
      if (Array.isArray(res.data)) {
        ...
        const transformedCities = res.data.data.map((item: any) => ({
          label: item.name,
          value: item._id,
        }));
  ...
  const handleChange = async (selectedCityIds: string[]) => {
    ...
    try {
      ...
      // Merge hotels from all cities
      const mergedHotels = allHotels.flatMap((res) => res.data?.data || []);
  ...
  return (
    <div className="flex flex-col gap-6 bg-white shadow-lg px-6 py-5 rounded-2xl border border-orange-200">
      ...
      >
        Click Here to Select Cities in India
      </button>
